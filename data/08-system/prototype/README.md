...
```bash
# 골든셋 회귀 검증 실행 — v1 축소판(9) + v2 확장(신규 트리거·경계 케이스)
python3 scorer.py --selftest

# v1 축소판 회귀만 (v2 확장 생략)
python3 scorer.py --selftest --v1only
...
| `run_golden_regression()` | v1 골든셋 3 triad(우수/보통/미흡) 축소판 회귀 | 부록 §6 |
| `run_v2_regression()` | v2 확장 회귀(신규 트리거 미흡 16 + 보통/우수 + LLM전용 xfail + 경계 4종) | v2 부록 §5 |
| `run_full_regression()` | v1 + v2 회귀를 함께 실행(`--selftest` 기본 경로) | — |

## S-3. 채점 규칙 (MockScorer)

1. **red flag 표현 감지 → 해당 요소 강제 '하'** (하드 트리거). 표현 테이블은 골든셋(v1+v2) 미흡
   앵커에서 추출(반공직·위법정당화·은폐책임전가·타인비하·답변포기 + 서면 대비 진술모순).
   **v2 부록 §5 신규 트리거 어구 25종을 보수적으로 추가**(정치중립 상대화·집단 비하·위법/특혜
   정당화·은폐/책임전가·평정심 상실 등). 오탐 방지를 위해 **긴 특정 어구**만 채택하고, 짧고
   중립적 표현(예: "정년이 보장되고", "확인하겠습니다")은 트리거로 쓰지 않는다.
...
6. **회귀 게이트**: 채점기/프롬프트/few-shot 변경 시 `run_full_regression()`(v1 축소판 9 + v2 확장
   27 하드 PASS + LLM 전용 xfail 4 + 경계 진단 2)으로 등급 뒤집힘·트리거 재현율·요소 일치율을
   검증(부록 §6·v2 부록 §5 통과 기준). 실제 LLM 채점기 도입 시에는 v2 부록 §5 목표대로 [D] xfail
   (허세·서면모순·침묵)의 미흡 감지와 [E] BC-1·BC-2의 교정 여부를 개선 지표로 추가 추적한다.

## 검증 결과 (골든셋 회귀 — v1 축소판 + v2 확장)

`python3 scorer.py --selftest` (3인 앙상블 J-STD·J-STR·J-JOB, 과반=2).

### (1) v1 축소판 회귀 — 등급 정확도 9/9 (100%)
...
- 3개 triad(반공직 / 타인비하 / 위법정당화+책임전가)로 주요 red flag 계열을 커버. 요소별 '하'
  위치도 골든셋 해설과 정합(Q1 미흡=헌신·윤리 하, Q9 미흡=창의·윤리 하 등).

### (2) v2 확장 회귀 — 하드 PASS 27/27, LLM 전용 xfail 4/4, known-limit 2건

v2 신규 20문항 + 경계 4종을 **규칙으로 판정 가능한 범위**만 회귀에 편입한다.

| 그룹 | 대상 | 판정 방식 | 결과 |
|---|---|---|---|
| **[A] red flag 미흡** | Q1·Q2·Q4·Q5·Q6·Q8·Q9·Q10·Q11·Q12·Q13·Q14·Q15·Q16·Q19·Q20 (16) | 신규 트리거 감지 → '미흡' 하드 PASS | **16/16 OK** |
| **[B] 결격 없는 보통** | Q1·Q6·Q9·Q13·Q17·Q20 보통 (6) | 오탐 없이 '보통' 하드 PASS | **6/6 OK** |
| **[C] 4요소 충분 우수** | Q1·Q8·Q13 우수 (3) | '우수' 하드 PASS | **3/3 OK** |
| **[D] LLM 전용 xfail** | Q7·Q17(허세 ⑤) · Q3(서면모순 ⑤) · Q18(지속침묵 ③) | 규칙 **미탐이 정상** → '보통' 유지 확인 | **4/4 미탐확인** |
| **[E] 경계 BC-3·BC-4** | 유창 반공직 / 버벅 진솔 | 하드 PASS(미흡 / 보통) | **2/2 OK** |
| **[E] 경계 BC-1·BC-2** | 짧지만 우수 / 길지만 공허 | **known-limitation 진단**(합/불 아님) | 진단 2건 |

- **하드 PASS 합계 36/36** (v1 9 + v2 27). 신규 트리거 25종 추가 후에도 v1 9/9 불변.
- **[D] LLM 전용**: 허세(㉯ "확실합니다"류 오답+단정)·서면모순(㉱ `written_task` 대조)·지속침묵(③)은
  단순 문자열 패턴이나 무맥락으로 감지 불가하다. 회귀는 이들을 **"MockScorer가 못 잡는 것이 정상"**
  인 xfail로 두고, 오히려 규칙이 '미흡'으로 잡으면(오탐) 경고한다. 이 경계가 "명시적 트리거는 잡되
  미묘한 판단은 LLM 몫"이라는 설계 원칙을 코드로 고정한다.
- **[E] known-limitation**: BC-1(짧지만 우수, 99자<150 → '보통' 과소평가)·BC-2(길지만 공허, 키워드
  나열로 헌신열정 요소 '상' 과대평가)는 **길이 게이트·키워드 카운트의 구조적 한계**를 드러내는
  지표다. 통과/실패로 채점하지 않고 진단만 출력하며, LLM 채점기 도입 시 교정 목표점으로 추적한다.
  BC-3·BC-4는 MockScorer도 정확히 처리해야 하는 항목(red flag 감지 / 유창성 미반영)이라 하드 PASS.
